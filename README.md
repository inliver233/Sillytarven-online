...
---
## 新仓库地址：[`zhaiiker/SillyTavernMOD`](https://github.com/zhaiiker/SillyTavernMOD/)（基于 1.16.0）。
...
- **OAuth第三方注册登录**：支持第三方账号快速注册和登录（V1.13.12 新增）
  ...
  - **一键登录**：无需记住密码，快速登录
  - **自动同步信息**：同步第三方账号的用户名与头像；Discord 按最小权限原则不申请或保存邮箱
...
- **永久账户**：账号密码、GitHub、Discord 和 Linux.do 注册创建的用户均为永久账户；邀请码只控制注册资格，不再设置账户期限
- **旧账户迁移**：服务启动时会自动将历史有限期账户升级为永久账户

### 🎫 邀请码系统新增功能 （V1.13.10 新增功能）
- **注册方式独立控制**：每种注册方式可单独开关，并可独立要求邀请码
- **永久激活**：邀请码使用后账号永久有效，不需要续费
- **历史兼容**：旧版本生成的限时邀请码仍可用于注册，但新账号会按永久账户创建
...
1. 注册账户
   ...
   - **方式二：OAuth第三方登录**（V1.13.12 新增/优化）
     ...
     - 系统会同步第三方账号的用户名与头像；Discord 登录注册不会申请或保存邮箱
